...
<#
    ...
    08/07/26  GL Fixed bug where not working for pwsh7.x due to bad interop declaration exposed in 7.x
#>
...
namespace PInvoke.Win32
{
    ...
    public static class Memory
    {
        ...
        public static extern bool SetProcessWorkingSetSizeEx( IntPtr proc, UIntPtr min, UIntPtr max , uint flags );
        [DllImport("kernel32.dll", SetLastError=true)]
        public static extern bool GetProcessWorkingSetSizeEx( IntPtr hProcess, ref UIntPtr min, ref UIntPtr max , ref uint flags );
    }
...
do
{
    ...
    # SIZE_T is pointer-sized. Convert script integer settings to native SIZE_T values.
    [UIntPtr]$nativeMinWorkingSet = if( $minWorkingSet -lt 0 ) { [UIntPtr]::op_Explicit( [UInt64]::MaxValue ) } else { [UIntPtr]::op_Explicit( [UInt64]$minWorkingSet ) }
    [UIntPtr]$nativeMaxWorkingSet = if( $maxWorkingSet -lt 0 ) { [UIntPtr]::op_Explicit( [UInt64]::MaxValue ) } else { [UIntPtr]::op_Explicit( [UInt64]$maxWorkingSet ) }
    ...
    {      
        ...
        if( $doIt )
        {
            ...
            if( $process.Handle )
            {
                if( $report )
                {
                    [UIntPtr]$thisMinimumWorkingSet = [UIntPtr]::Zero
                    [UIntPtr]$thisMaximumWorkingSet = [UIntPtr]::Zero
                    [uint]$thisFlags = 0 ## Grammar alert! :-)
                    ...
                    if( $result )
                    {
                        ...
                        $null = $results.Add( ([pscustomobject][ordered]@{ 'Name' = $process.Name ; 'PID' = $process.Id ; 'Handle Count' = $process.HandleCount ; 'Start Time' = $process.StartTime ;
                                ...
                                'Minimum Working Set (KB)' = ( $thisMinimumWorkingSet.ToUInt64() / 1KB ) ; 'Maximum Working Set (KB)' = ( $thisMaximumWorkingSet.ToUInt64() / 1KB )
                ...
                elseif( $pscmdlet.ShouldProcess( $action , 'Trim' ) ) ## Handle may be null if we don't have sufficient privileges to that process
                {
                    ...
                    [bool]$result = [PInvoke.Win32.Memory]::SetProcessWorkingSetSizeEx( $process.Handle,$nativeMinWorkingSet,$nativeMaxWorkingSet,[uint32]$flags);$LastError = [ComponentModel.Win32Exception][Runtime.InteropServices.Marshal]::GetLastWin32Error()
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUG2oT5BfoZt+y4i7pzhyYfydO
# jdeggh9gMIIFfTCCA2WgAwIBAgIQAdazdTZfIM2RHdcv5fmTZDANBgkqhkiG9w0B
...
# FTAjBgkqhkiG9w0BCQQxFgQUgCK+UpsUpCPyZ1xccfduKLCBxSMwCwYHKoZIzj0C
# AQUABGcwZQIxALlFKtscW/ePUsed38LUEeOo5jA0Jkve4SE8MnhnD+aGgmJxHtuH
# 9vY50nvYeFPDGwIwONSX75+YdIPDUZ0TXNkdV7sFgGuPsEPNi/yYvX+lRaFTAj4J
# Sl096gZ1ABJs7jaPoYIDJjCCAyIGCSqGSIb3DQEJBjGCAxMwggMPAgEBMH0waTEL
...
# SIb3DQEJAzELBgkqhkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTI2MDcwODE3MDAw
# N1owLwYJKoZIhvcNAQkEMSIEIH7eg1X0Edi2ysB3l4LCLe5NG4xwC0MnYhoFFgnC
# 11jKMA0GCSqGSIb3DQEBAQUABIICAHgj+UtIzJbe0I4EcSXHG7KctuxK7dn5uzMJ
# kVX0KHQA0stQx9quhfmALK0IdHyBE0ZJ++o85/m4zU6ofAAxGHWXGwKSxf0vcgVz
# BCxIEfhhMuSNbhuR2gIlWKyNQxniyz1w0IbdPyygwVQCQT8JLcQlkKLZ7jUtTsQe
# 4yc6jh8ixZHBACahfNyzMHvVP8zsrwg7maDPFxgatZmzDAKQZyuSuZH+qCyyoudf
# fXRuF1XowfsXmVx4h687WFbFGpXcfdvvrT0/u6J8sl71JEL8kJnjKkZBMiYOow3L
# VbKb7USHTigGJgTgELEhup+WMWi1/1SyphavAA12ATgFPOuSZsdrFEU5eauTIwti
# KKz0SLQVyRXNP7NiIIm5yy84RyP8k4YJ2IR5WP07BBokk+tgUl9AdD72tMyMdLmj
# Z6dZax5YPBTAp91riCLY3riwPO7jN+e+06P12p/oba4lSwARVvBOuBReTx1hFVcX
# GZASIP9sCaOV/QUgMZ9PMcwq7fe4rz6CKLr3nMvzv1pqKb1ojX4Du83pONa1UzLR
# 70LvsEfXA5OX886o35yo5Rp9sCz8c9c2IJBKlZFD91lLsUjI5Ab12ork5sFDIYed
# 6DBQOtgHSk8ZcIPOPn/XDON4ykzqGX9AYo6p8zsguXYlg6CzPXX8JofjR5+kS66A
# cDngA4ir
