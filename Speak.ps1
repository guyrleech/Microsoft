<#
.SYNOPSIS
    Speak words, phrases or sentences using built in Microsoft Text to Speech. Will unmute and change volume level temporarily if specified.

.PARAMETER speech
    Word/phrases/sentences to say

.PARAMETER voice
    The voice to use for the speaking

.PARAMETER volumePercent
    Increase the system audio output volume to this level for the speaking and return it to its previous value afterwards

.PARAMETER unmute
    Unmute the default output if it is muted and mute it again afterwards

.PARAMETER outputfile
    Record the speech into the specified file, if it does not exist or -clobber is specified

.PARAMETER clobber
    If the file specified by -outputfile already exists, overwrite it otherwise the script will fail

.EXAMPLE
    & '.\Speak.ps1' -speech "Finished" -volumePercent 100
    
    Play the word "finished" at 100% of volume, returning the volume to its previous level afterwards. No sound will be heard if the speaker is muted
    
.EXAMPLE
    & '.\Speak.ps1' -speech "Finished" -volumePercent 100 -unmute
    
    Play the word "finished" at 100% of volume, unmuting the speaker if it is muted and muting it again after the speech and returning the volume to its previous level

.EXAMPLE
    & '.\Speak.ps1' -speech "Finished" -outputfile c:\temp\finished.wav

    Record the word "finished" to the file c:\temp\finished.wav if it does not already exist

.NOTES
    With code from Andy Morgan and Joseph Moody

    Modification History:

    2023/04/13  @guyrleech  Initial release
#>
...
Param
(
    [Parameter(Mandatory=$true,HelpMessage='Text to speak',ValueFromPipeline = $true)]
    [string[]]$speech ,
...
if( -Not [string]::IsNullOrEmpty( $outputFile ) -and (Test-Path -Path $outputFile) -and -Not $clobber )
{
    Throw "Outut file `"$outputFile`" already exists and -clobber not specified"
}

## https://deployhappiness.com/setting-sound-volume-unmuting-powershell/

if( -Not ([System.Management.Automation.PSTypeName]'Audio').Type -and -Not $PSBoundParameters[ 'outputFile' ] )
{
...
if( $unmute -or $outputFile -or -Not [audio]::Mute )
{
    ## https://twitter.com/andyjmorgan/status/1644371975215669253
    ...
    [decimal]$currentVolume = -1
    [bool]$unmuted = $false
    
    if( $synth = New-Object -TypeName System.Speech.Synthesis.SpeechSynthesizer )
    {
        if( [string]::IsNullOrEmpty( $outputFile ) )
        {
            if( $volumePercent -gt 0 )
            {
                $currentVolume = [audio]::Volume
                [audio]::Volume = $volumePercent / 100
            }

            if( $unmute -and [audio]::Mute )
            {
                [audio]::Mute = $false
                $unmuted = $true
            }
        ...
        ForEach( $line in $speech )
        {
	        $synth.Speak( $line )
        }
    }

    ## restore previous audio values
    ...
    if( $outputFile )
    {
        $synth.SetOutputToDefaultAudioDevice() ## otherwise file stays open
    }
}
else
{
    Write-Warning -Message "Muted so not playing"
}
 
# SIG # Begin signature block
# MIIjcAYJKoZIhvcNAQcCoIIjYTCCI10CAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQ