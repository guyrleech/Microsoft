#requires -version 3
<#
    Show current folder redirections or redirect specified known folders to specified path, creating directory if necessary.

    See results in HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Shell Folders
...
A comma separated list of strings of the form FolderId=Path. If none specified then current folder redirections will be returned

.EXAMPLE

& '.\Redirect Folder.ps1'

Shows current folder redirections for the user running the script

.EXAMPLE

& '.\Redirect Folder.ps1' -folders Desktop=H:\Desktop,Documents=H:\Documents

Redirect the Desktop to H:Desktop and My Documents to H:\Documents

.NOTES

Environment variables like %UserName% and %UserProfile% can be specified in the path
...
## https://docs.microsoft.com/en-gb/windows/desktop/shell/knownfolderid
[hashtable]$knownfolderids = @{
    ...
    'Computer'='0AC0837C-BBF8-452A-850D-79D08E667CA7'
    'Conflict'='4BFEFB45-347D-4006-A5BE-AC0CB0567192'
    'Connections'='6F0CD92B-2E97-45D1-88FF-B0D186B8DEDD'
...
{
    $redirections = [Environment+SpecialFolder]::GetNames([Environment+SpecialFolder])| Sort | ForEach-Object{ [pscustomobject][ordered]@{ 'KnownFolder' = $_ ; 'Path' = [Environment]::GetFolderPath($_) } } 
    
    if( $gridView )
    {
        $selected = $redirections | Out-GridView -Title "$($redirections.Count) folder redirections" -PassThru
        if( $selected )
        {
            $selected | clip.exe
        }
...
else
{
    ...
    {
        [string]$knownFolder,[string]$path = $folder -split '='

        Write-Verbose "Redirecting $knownFolder to `"$path`""

        if( [string]::IsNullOrEmpty( $knownFolder ) -or [string]::IsNullOrEmpty( $path ) )
        {
            Write-Warning "Bad syntax for `"$folder`""
        }
        else
        {
            $knownFolderId = $knownfolderids[ $knownFolder ]

            if( ! $knownFolderId )
            {
                Write-Warning "Unable to find id for known folder `"$knownFolder`""
            }
            else
            {
                ...
                if( ! (Test-Path -Path $path -PathType Container -ErrorAction SilentlyContinue ) )
                {
                    ...
                    if( ! $newFolder )
                    {
                        Write-Warning "Failed to create folder `"$path`" for $knownFolder"
                    }
                }

                $returned = [Win32Interop]::SHSetKnownFolderPath( [ref]$knownFolderId , 0 , 0 , $path ); $lastError = [ComponentModel.Win32Exception][Runtime.InteropServices.Marshal]::GetLastWin32Error()

                if( $returned )
                {
                    Write-Warning "Failed to redirect $knownFolder to `"$path`" - $lastError"
                    $failures++
                }
            }
        }
